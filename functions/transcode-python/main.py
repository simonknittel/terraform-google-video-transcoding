...
def main(event, context):
    ...
    download(input_bucket, event, input_file)
    transcode(input_file, local_h264_output_file)
    upload(storage_client, bucket_h264_output_file, local_h264_output_file)
    clean_up(input_file, local_h264_output_file)
    email(bucket_h264_output_file)

def download(input_bucket, event, input_file):
    print("Downloading ...")
    input_blob = input_bucket.get_blob(event['name'])
    input_blob.download_to_filename(input_file)

def transcode(input_file, local_h264_output_file):
    print("Transcoding ...")
...
def clean_up(input_file, local_h264_output_file):
    print("Cleaning up ...")
